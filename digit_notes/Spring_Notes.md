...
```
## hashCode(): //To be worked on again by me
hashCode() is not the actual memory address; it is an integer value generated by the JVM to represent the object's identity (by default in Object).

Memory address was historically used to generate hashCode() because each object has a unique location in memory, making it a simple way to create a unique identity value.
```
import java.util.*;
